...
impl FilterService {
    ...
    pub fn init_tables(&mut self){
        ...
        iferror(self.tables_v4.append_unique("filter", "INPUT", &("-j".to_string()+" "+BANDWIDTH_IN_CHAIN)[..]));
        iferror(self.tables_v4.append_unique("filter", "OUTPUT", &("-j".to_string()+" "+BANDWIDTH_OUT_CHAIN)[..]));

        iferror(self.tables_v4.append_unique("filter", "INPUT", &("-j".to_string()+" "+BLACKLIST_CHAIN)[..]));
        iferror(self.tables_v4.append_unique("filter", "INPUT", &("-j".to_string()+" "+PROTECT_CHAIN)[..]));
        // let ip = ip::local_ip());
        // 本机ip放行
        ...
        iferror(self.tables_v4.append_unique("filter", PROTECT_CHAIN, &("-p".to_string()+ " icmp -j ACCEPT")[..]));
        ...
        if self.blacklisted_ports.len() >0 {
            for (port,_) in self.blacklisted_ports.iter(){
                iferror(self.tables_v4.append_unique("filter", PROTECT_CHAIN, &("-p tcp --dport ".to_string()+&port.to_string()[..]+" -j NFLOG --nflog-group 100 --nflog-prefix "+ PREFIX_DEFAULT)[..]));
                iferror(self.tables_v4.append_unique("filter", PROTECT_CHAIN, &("-p udp --dport ".to_string()+&port.to_string()[..]+" -j NFLOG --nflog-group 100 --nflog-prefix "+ PREFIX_DEFAULT)[..]));

                if valid_trigger {
                    log::info!("valid_trigger: {pstr}, {tstr}, {valid_trigger}");
                    iferror(self.tables_v4.append_unique("filter", PROTECT_CHAIN, &("-p tcp --dport ".to_string()+&port.to_string()[..]+" -m recent --name "+&port.to_string()[..]+"TRIGGER --set")[..]));
                    iferror(self.tables_v4.append_unique("filter", PROTECT_CHAIN, &("-p tcp --dport ".to_string()+&port.to_string()[..]+" -m recent --name "+&port.to_string()[..]+"TRIGGER --rcheck --seconds "+ &tstr[..] +" --hitcount "+ &pstr[..] +" -j NFLOG --nflog-group 100 --nflog-prefix  "+PREFIX_TRIGGER)[..]));
                }
                iferror(self.tables_v4.append_unique("filter", PROTECT_CHAIN, &("-p tcp --dport ".to_string()+&port.to_string()[..]+" -j "+ &self.deny_action)[..]));
                iferror(self.tables_v4.append_unique("filter", PROTECT_CHAIN, &("-p udp --dport ".to_string()+&port.to_string()[..]+" -j "+ &self.deny_action)[..]));
            }
        }else{
                // 不指明端口时，全端口防护
                iferror(self.tables_v4.append_unique("filter", PROTECT_CHAIN, &("-j ".to_string()+&self.deny_action)[..]));
        }
    ...
    pub fn clear_tables(&mut self){
        ...
        iferror(self.tables_v4.delete("filter", "INPUT", &("-j ".to_string()+BANDWIDTH_IN_CHAIN)[..]));
        iferror(self.tables_v4.delete("filter", "OUTPUT", &("-j ".to_string()+BANDWIDTH_OUT_CHAIN)[..]));
        iferror(self.tables_v4.delete("filter", "INPUT", &("-j ".to_string()+PROTECT_CHAIN)[..]));      
        iferror(self.tables_v4.delete("filter", "INPUT", &("-j ".to_string()+BLACKLIST_CHAIN)[..])); 
    ...
    pub fn load_config(&mut self, config :crate::Config){
        // self.
        ...
        if config.reverse_proxy{
            ...
            self.whitelisted_ports.insert(config.listen_port, empty_struct {  });
        }
    ...
    pub fn del_whitelisted_ip(&mut self,ip:cidr_utils::cidr::IpCidr){
        ...
        iferror(self.tables_v4.delete("filter", BANDWIDTH_IN_CHAIN, &(" -s ".to_string()+&ip.to_string()[..]+" -j RETURN")[..]));
        iferror(self.tables_v4.delete("filter", BANDWIDTH_OUT_CHAIN, &(" -d ".to_string()+&ip.to_string()[..]+" -j RETURN")[..]));
        iferror(self.tables_v4.delete("filter", PROTECT_CHAIN, &(" -s ".to_string()+&ip.to_string()[..]+" -j ACCEPT")[..]));
    ...
    pub fn del_blacklisted_ip(&mut self,ip:cidr_utils::cidr::IpCidr){
        ...
        iferror(self.tables_v4.delete("filter", BLACKLIST_CHAIN, &(" -s ".to_string()+&ip.to_string()[..]+" -j "+ &self.deny_action)[..]));
    ...
    pub fn get_whitelist_data(&mut self) -> Vec<WhitelistRecord> {
        ...
        if input_raw.status.success() && output_raw.status.success()  {
            ...
            for (ip,_) in self.whitelisted_ips.iter(){
                ...
                .filter(|&line| line.contains(&ip.to_string()[..]))
                .collect();
                        lines_out = stdout_output_v6.lines()
                .filter(|&line| line.contains(&ip.to_string()[..]))
                .collect();
                    }
                }
